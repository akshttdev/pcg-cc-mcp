    ...
    vec![
            ...
            workflows: vec![
                AgentWorkflow {
                    ...
                    stages: vec![
                        ...
                        WorkflowStage {
                            name: "Scene Analysis".to_string(),
                            description: "Deep FFmpeg analysis of every clip — brightness, motion intensity, complexity, content classification (high_energy, establishing, intimate, transition, ambient). Must run BEFORE hero selection.".to_string(),
                            output: "Per-clip scene analysis with content map".to_string(),
                        },
                        WorkflowStage {
                            name: "Sonic Engineering".to_string(),
                            description: "Sonix analyzes the operator-selected music track — BPM detection, beat grid, energy curve, section identification (intro/verse/chorus/bridge/outro), transition markers for beat-locked cuts.".to_string(),
                            output: "Beat grid + section map + transition markers".to_string(),
                        },
                        WorkflowStage {
                            name: "Visual QC".to_string(),
                            description: "Spectra sends keyframes to Vision API (provider-agnostic), scores composition, selects optimal in-points per clip. Prevents cut-off subjects and poor framing.".to_string(),
                            output: "Optimized in-points + composition scores".to_string(),
                        },
                        WorkflowStage {
                            name: "Smart Assembly".to_string(),
                            description: "Match clips to music sections by energy/content affinity, beat-lock all cuts to nearest beat, mute NAT audio (music only on A1), generate narrative arc (establish → build → peak → resolve).".to_string(),
                            output: "Beat-locked timeline with music-driven shot selection".to_string(),
                        },
                        WorkflowStage {
                            name: "Export + Color".to_string(),
                            description: "Export Premiere Pro XML (FCP XML 5) with beat markers, section markers, LUT presets, and volume automation. Ready for NLE import.".to_string(),
                            output: "Premiere Pro XML project file".to_string(),
                        },
                    ],
                    deliverables: vec![
                        "Premiere Pro XML with beat-locked edit".to_string(),
                        "Recap v1 (1080x1920 + 1920x1080)".to_string(),
                        "Shot list + suggested captions".to_string(),
                    ],
                    automation_stack: vec![
                        "Dropbox ingestion".to_string(),
                        "FFmpeg scene analysis (signalstats, scene detection)".to_string(),
                        "FFmpeg beat/energy analysis (volumedetect, ebur128)".to_string(),
                        "Vision API composition scoring (provider-agnostic)".to_string(),
                        "RecapAssemblyEngine (beat-locked cuts)".to_string(),
                        "Premiere XML exporter".to_string(),
                    ],
                    training_assets: vec![
        ...
        AgentProfile {
            ...
            capabilities: vec![
                ...
                "visual_quality_control".to_string(),
            ],
            strengths: vec![
            ...
            workflows: vec![
                ...
                AgentWorkflow {
                    workflow_id: "visual-qc-pass".to_string(),
                    name: "Visual QC Pass".to_string(),
                    objective: "Analyze video frames via Vision API to score composition, select optimal in-points, and prevent cut-off subjects before assembly.".to_string(),
                    trigger_keywords: vec![
                        "visual qc".to_string(),
                    ...
                    stages: vec![
                        WorkflowStage {
                            name: "Frame Extraction".to_string(),
                            description: "Extract candidate keyframes from each clip using FFmpeg at evenly-spaced timestamps.".to_string(),
                            output: "Candidate frame JPEGs".to_string(),
                        },
                        WorkflowStage {
                            name: "Vision Analysis".to_string(),
                            description: "Send frames to Vision API (provider-agnostic) and score composition, subject framing, headroom, exposure, and sharpness.".to_string(),
                            output: "Per-frame analysis scores".to_string(),
                        },
                        WorkflowStage {
                            name: "In-Point Optimization".to_string(),
                            description: "Select best in-point per clip based on composition scores, apply to footage clips, and generate QC report.".to_string(),
                            output: "Optimized in-points + QC report".to_string(),
                        },
                    ],
                    deliverables: vec![
                        "Visual QC result with per-clip scores".to_string(),
                        "Optimized in-points applied to footage".to_string(),
                    ],
                    automation_stack: vec![
                        "Vision API (provider-agnostic)".to_string(),
                        "FFmpeg frame extraction".to_string(),
                        "Editron VisualQcEngine".to_string(),
                    ],
                    training_assets: vec![
                        "Composition scoring rubric".to_string(),
                        "Brand framing guidelines".to_string(),
                    ],
