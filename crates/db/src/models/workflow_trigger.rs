...
impl WorkflowTrigger {
    pub async fn create(
        pool: &SqlitePool,
        input: CreateWorkflowTrigger,
    ) -> Result<Self, sqlx::Error> {
        let id = Uuid::new_v4().to_string();
        let trigger_type = input
            .trigger_type
            .unwrap_or_else(|| "data_source_created".to_string());
        let filter_ds_types = input
            .filter_data_source_types
    ...
    pub async fn find_all(pool: &SqlitePool) -> Result<Vec<Self>, sqlx::Error> {
        sqlx::query_as::<_, Self>("SELECT * FROM workflow_triggers ORDER BY created_at DESC")
            .fetch_all(pool)
            .await
    }
    ...
    pub fn is_past_cooldown(&self) -> bool {
        ...
        match &self.last_triggered_at {
            Some(last) => {
                ...
                if let Ok(last_time) =
                    chrono::NaiveDateTime::parse_from_str(last, "%Y-%m-%d %H:%M:%S%.f").or_else(
                        |_| chrono::NaiveDateTime::parse_from_str(last, "%Y-%m-%dT%H:%M:%S%.f"),
                    )
                {
        ...
        let due: Vec<Self> = enabled
            ...
            .filter(|trigger| {
                ...
                match &trigger.last_triggered_at {
                    Some(last) => {
                        if let Ok(last_time) =
                            chrono::NaiveDateTime::parse_from_str(last, "%Y-%m-%d %H:%M:%S%.f")
                                .or_else(|_| {
        ...
        let matching = enabled
            ...
            .filter(|trigger| {
                ...
                if let Some(ref types_json) = trigger.filter_data_source_types
                    ...
                    && !types.is_empty()
                    && !types.contains(&data_source_type.to_string())
                {
                    return false;
                }
