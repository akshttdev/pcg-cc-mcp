...
-- Step 1: Create new stages for each Clients pipeline
-- We insert new stages with unique names, then remap deals, then delete old stages.

-- For each pipeline of type 'clients', do the migration.
-- SQLite doesn't support procedural loops, so we use a CTE-based approach.

-- Create a temp table with the new stage definitions
...
INSERT INTO _temp_new_client_stages (name, color, position, is_closed, is_won, probability) VALUES
    ('Lead',              '#6B7280', 0, 0, 0, 5),
    ('Business Analysis', '#3B82F6', 1, 0, 0, 20),
    ('Discovery',         '#8B5CF6', 2, 0, 0, 40),
    ('Build Proposal',    '#F59E0B', 3, 0, 0, 55),
    ('Polish',            '#EC4899', 4, 0, 0, 70),
    ('Proposal Meeting',  '#EF4444', 5, 0, 0, 85),
    ('Closed Won',        '#22C55E', 6, 1, 1, 100),
    ('Closed Lost',       '#9CA3AF', 7, 1, 0, 0);

-- Insert new stages for all clients pipelines (skip if stage name already exists for that pipeline)
...
-- Delete old stages that no longer exist (Qualified, Proposal, Negotiation)
DELETE FROM crm_pipeline_stages
WHERE id IN (
    SELECT s.id FROM crm_pipeline_stages s
    JOIN crm_pipelines p ON p.id = s.pipeline_id
    WHERE p.pipeline_type = 'clients'
    AND s.name IN ('Qualified', 'Proposal', 'Negotiation')
);

-- Update positions on existing Lead and Closed Won/Lost stages to match new ordering
