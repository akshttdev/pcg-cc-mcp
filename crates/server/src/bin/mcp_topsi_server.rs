...
use server::mcp::topsi_server::TopsiServer;
...
fn main() -> anyhow::Result<()> {
    ...
    sentry::configure_scope(|scope| {
        scope.set_tag("source", "mcp_topsi");
    });
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async {
            ...
            tracing::debug!("[MCP] Starting Topsi MCP server version {version}...");

            let db_service = DBService::new().await?;
            let pool = db_service.pool.clone();

            match TopsiServer::new(pool).serve(stdio()).await {
                Ok(service) => {
