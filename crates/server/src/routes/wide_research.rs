...
async fn get_session(
    ...
    let session_id = Uuid::parse_str(&session_id).map_err(|_| ApiError::BadRequest("Invalid UUID".into()))?;
    let pool = &deployment.db().pool;
...
async fn start_subagent(
    ...
    let session_id = Uuid::parse_str(&session_id).map_err(|_| ApiError::BadRequest("Invalid UUID".into()))?;
    let subagent_id = Uuid::parse_str(&subagent_id).map_err(|_| ApiError::BadRequest("Invalid UUID".into()))?;
    let pool = &deployment.db().pool;
