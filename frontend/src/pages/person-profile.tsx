...
function OverviewTab({
  ...
  person: CrmContactRecord & Record<string, unknown>;
  companyRoles: PersonCompanyRole[];
}) {
  const emails = parseJson<{ value: string; label: string }[]>(
    person.emails as string | undefined,
    []
  );
  const phones = parseJson<{ value: string; label: string }[]>(
    person.phones as string | undefined,
    []
  );
  ...
  const intelRaw = parseJson<Record<string, unknown>>(
    person.intelligence_raw as string | undefined,
    {}
  );
  const keyFacts = [
          ...
          ? emails.map((e, i) => (
              <div key={i} className="flex items-center gap-3 text-sm">
                ...
                <div className="min-w-0">
                  <a
                    href={`mailto:${e.value}`}
                    className="text-primary hover:text-primary/80 truncate block"
                  >
                    {e.value}
                  </a>
                  {e.label && (
          ...
          ? phones.map((p, i) => (
              <div key={i} className="flex items-center gap-3 text-sm">
                ...
                <div className="min-w-0">
                  <a
                    href={`tel:${p.value}`}
                    className="text-foreground hover:text-foreground truncate block"
                  >
                    {p.value}
                  </a>
                  {p.label && (
      ...
      <div className="rounded-lg border border-border/80 bg-card p-5 shadow-md space-y-3">
        ...
        {person.lifecycle_stage && (
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge
              variant="outline"
              className={`text-xs ${lifecycleColor(person.lifecycle_stage)}`}
            >
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-24 h-1.5 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(person.lead_score, 100)}%` }}
                />
              </div>
              <span className="text-xs text-foreground w-7 text-right">
                {person.lead_score}
              </span>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-border">
            <div className="flex flex-wrap gap-1.5">
              {tags.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-muted text-xs text-muted-foreground"
                >
      ...
      <div className="rounded-lg border border-border/80 bg-card p-5 shadow-md">
        ...
        {person.intelligence_confidence > 0 && (
          <div className="mb-3">
            ...
            <div className="h-1.5 bg-muted rounded-full overflow-hidden">
              <div
                className="h-full bg-indigo-500 rounded-full"
                style={{ width: `${person.intelligence_confidence * 100}%` }}
              />
            </div>
          </div>
        )}
        ...
        {person.intelligence_last_run_at && (
          <p className="text-xs text-muted-foreground/70 mt-3">
            Last run {fmtDate(person.intelligence_last_run_at)}
          </p>
        )}
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {profiles.map((p) => (
        <div
          key={p.id}
          className={`rounded-xl border p-5 ${platformColors[p.platform] ?? 'bg-card border-border'}`}
        >
          ...
          <div className="mt-3 flex items-center gap-4 text-xs opacity-70">
            {p.follower_count != null && (
              <span>
                <span className="font-semibold text-sm opacity-100">
          ...
          {p.bio && (
            <p className="text-xs opacity-60 leading-relaxed mt-2 line-clamp-3">
              {p.bio}
            </p>
          )}
          ...
          {p.last_synced_at && (
            <p className="text-xs opacity-40 mt-2">
              Synced {fmtDate(p.last_synced_at)}
            </p>
          )}
        </div>
      ))}
    ...
    <div className="space-y-4">
      ...
      {notes.length === 0 ? (
        ...
        notes.map((note) => (
          <div
            key={note.id}
            className="rounded-lg border border-border/80 bg-card p-4 shadow-md"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    onClick={() =>
                      updateMut.mutate({ id: note.id, text: editText })
                    }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {passes.length} research pass{passes.length !== 1 ? 'es' : ''}{' '}
          completed
        </p>
        <Button
          size="sm"
      ...
      {passes.length === 0 ? (
        <EmptyState
          icon={Layers}
          title="No research passes yet"
          description="Run the first pass to start building intelligence."
        />
      ) : (
        passes.map((pass) => {
          ...
          return (
            <div
              key={pass.id}
              className="rounded-lg border border-border/80 bg-card p-5 shadow-md"
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-sm font-medium capitalize ${focusColors[pass.research_focus ?? ''] ?? 'text-foreground'}`}
                  >
    ...
    <div className="space-y-4">
      ...
      {reports.length === 0 ? (
        <EmptyState
          ...
          description="Generate a business analytics report for this lead."
        />
      ) : (
        reports.map((r) => (
          <Link
...
export function PersonProfilePage() {
  ...
  const [tab, setTab] = useState<
    'overview' | 'social' | 'notes' | 'research' | 'reports'
  >('overview');
  ...
  if (!person)
    return <div className="p-8 text-muted-foreground">Contact not found.</div>;
  ...
  const primaryCompany =
    personAny.company_roles?.find(
      (r: PersonCompanyRole) => r.is_primary === 1
    ) ?? personAny.company_roles?.[0];
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap text-sm text-muted-foreground">
              ...
              {person.job_title && (person.company_name || primaryCompany) && (
                <span className="text-muted-foreground/70">@</span>
              )}
              {primaryCompany ? (
                <Link
                  to={`/companies/${primaryCompany.company_id}`}
                  className="flex items-center gap-1 text-primary hover:text-primary/80"
                >
                  <Building2 className="w-3.5 h-3.5" />{' '}
                  {primaryCompany.company_name ?? person.company_name}
                </Link>
              ) : person.company_name ? (
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {personAny.company_id && (
                <Link to={`/companies/${personAny.company_id}`}>
                  <Badge
                    variant="outline"
                    className="text-xs border-border text-muted-foreground hover:border-indigo-700 hover:text-indigo-400 cursor-pointer"
                  >
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <TabBtn active={tab === 'social'} onClick={() => setTab('social')}>
          Social{' '}
          {(personAny.social_profiles?.length ?? 0) > 0 &&
            `(${personAny.social_profiles.length})`}
        </TabBtn>
        ...
        <TabBtn active={tab === 'research'} onClick={() => setTab('research')}>
          Research{' '}
          {(person.research_pass_count ?? 0) > 0 &&
            `(${person.research_pass_count})`}
        </TabBtn>
      ...
      {tab === 'overview' && (
        <OverviewTab
          person={person as CrmContactRecord & Record<string, unknown>}
          companyRoles={personAny.company_roles ?? []}
        />
      )}
      {tab === 'social' && (
        <SocialTab profiles={personAny.social_profiles ?? []} />
      )}
