    ...
    <>
      ...
      <div className="pointer-events-auto absolute bottom-4 left-4 w-[min(30rem,calc(100%-2rem))]">
        <div className="rounded-lg border border-amber-600/60 bg-[#1b1209]/90 shadow-[0_12px_40px_rgba(0,0,0,0.6)]">
          <div className="flex items-center justify-between border-b border-amber-500/40 px-3 py-1 text-xs uppercase tracking-[0.3em] text-amber-200">
            <div className="flex flex-wrap items-center gap-2 font-semibold">
              {['All', 'Grid', 'Direct', 'System'].map((label) => (
                <span
                  key={label}
                  className="rounded border border-amber-500/50 bg-black/30 px-2 py-0.5 text-xs tracking-[0.2em]"
                >
                  {label}
                </span>
              ))}
          ...
          <div className="border-b border-amber-500/30 px-3 py-2 text-xs text-amber-100/80">
            {noraLine}
          </div>
          <div
            className={cn(
          ...
          >
            <AgentChatConsole
              className="rounded-none border-0 bg-[#0b0905]/90 text-sm text-amber-100"
              statusLine={noraLine}
          ...
          <div className="border-t border-amber-500/30 px-3 py-1 text-xs text-amber-200/80">
      ...
      <div className="pointer-events-auto absolute bottom-4 right-4 flex flex-col items-end gap-3">
        {activeHudPanel && (
          <div className="w-[min(34rem,calc(100%-2rem))] rounded-2xl border border-amber-500/60 bg-[#080705]/95 shadow-[0_20px_45px_rgba(0,0,0,0.65)]">
            <div className="flex items-center justify-between border-b border-amber-500/40 px-5 py-3 text-xs uppercase tracking-[0.3em] text-amber-100">
              <div>
                <p className="text-sm font-semibold tracking-[0.2em]">{HUD_PANEL_META[activeHudPanel].title}</p>
                <p className="text-xs tracking-[0.15em] text-amber-200/70">
              ...
              <button
                ...
                className="rounded border border-amber-500/40 px-3 py-1 text-xs tracking-[0.2em] text-amber-100 transition hover:bg-amber-500/20"
              >
                CLOSE
              </button>
            </div>
          ...
          {HUD_NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex flex-col items-center gap-1 rounded-md px-3 py-1 text-xs tracking-[0.2em] transition',
                  isActive
