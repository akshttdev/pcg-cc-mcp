...
export function SocialContentView({
  ...
  const PostPill = ({
    ...
    return (
      <div
        className={`flex items-center gap-1 px-1.5 py-0.5 rounded border text-xs leading-tight truncate ${statusColor}`}
      >
        <Icon
                  ...
                  <div className="p-1.5 space-y-1 flex-1">
                    {dayPosts.map((post) => {
                      ...
                      return (
                        <div
                          key={post.id}
                          className={`rounded border p-1.5 cursor-default group relative ${statusColor}`}
                        >
