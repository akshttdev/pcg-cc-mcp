...
export function SocialOverviewView({
  ...
  const agg = useMemo(() => {
    ...
    mentionQueries.forEach((q, i) => {
      q.data?.forEach((m) =>
        allMentions.push({ ...m, _project: projectEntries[i].name })
      );
    });
    ...
    const urgent = allMentions.filter(
    ...
    allAccounts.forEach((a) => {
      if (!byPlatform[a.platform])
        byPlatform[a.platform] = { accounts: 0, followers: 0 };
        ...
        {[
          {
            ...
            value: formatCompactNumber(agg.totalFollowers),
            icon: Users,
            color: 'text-blue-500',
            action: () => onSwitchView('accounts'),
          },
          {
            ...
            action: () => onSwitchView('content'),
          },
          {
            label: 'Unread Mentions',
            value: agg.unread,
            icon: Inbox,
            color: agg.unread > 0 ? 'text-amber-500' : 'text-muted-foreground',
            action: () => onSwitchView('inbox'),
          },
          {
            label: 'Published',
            value: agg.published,
          ...
          <Card
            key={label}
            className="bg-card/80 backdrop-blur-sm border-border/50 cursor-pointer hover:border-border transition-colors"
            onClick={action}
          >
              ...
              <div className="space-y-2">
                ...
                {agg.brandHandles.map(({ platform, handle }) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`h-4 w-4 shrink-0 ${PLATFORM_COLORS[platform] || 'text-muted-foreground'}`}
                      />
              ...
              <div className="space-y-2">
                {Object.entries(agg.byPlatform).map(([platform, data]) => {
                  ...
                  return (
                    <div
                      key={platform}
                      className={`flex items-center gap-2.5 p-2 rounded-lg ${PLATFORM_BG[platform] || 'bg-muted/40'}`}
                    >
                      <Icon
                        className={`h-4 w-4 shrink-0 ${PLATFORM_COLORS[platform] || 'text-muted-foreground'}`}
                      />
                      ...
                      <span className="text-xs text-muted-foreground">
                        {data.accounts} acct{data.accounts !== 1 ? 's' : ''}
                      </span>
                    </div>
                  );
                })}
                    ...
                    >
                      ...
                      {(post.likes > 0 || post.impressions > 0) && (
                        <div className="text-right shrink-0">
                          {post.impressions > 0 && (
                            <p className="text-xs text-muted-foreground">
                              {formatCompactNumber(post.impressions)} views
                            </p>
                          )}
                          {post.likes > 0 && (
                            <p className="text-xs text-muted-foreground">
            ...
            <CardGrid columns={{ md: 2, lg: 3 }} gap={2}>
              {agg.recentMentions.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`p-2.5 rounded-lg border border-border/40 ${m.status === 'unread' ? 'bg-accent/20' : ''}`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <Icon
                        className={`h-3.5 w-3.5 shrink-0 ${PLATFORM_COLORS[m.platform] || 'text-muted-foreground'}`}
                      />
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      {m.sentiment && m.sentiment !== 'unknown' && (
                        <span
                          className={`text-[9px] ${SENTIMENT_COLORS[m.sentiment]}`}
                        >
                          {m.sentiment}
                        </span>
                      )}
