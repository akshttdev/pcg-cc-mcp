...
export function ModelsSettings() {
  ...
  const pricingByProvider = useMemo(() => {
    ...
    Object.values(grouped).forEach((arr) => arr.sort((a, b) => a.model.localeCompare(b.model)));
    return grouped;
  }, [modelPricing]);

  // Format cost as dollars per million tokens
  const formatCost = (centsPer1M: number) => {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure AI models and executor profiles used by coding agents. Each executor type (Claude Code, Gemini, etc.) can have multiple named configurations.
          </CardDescription>
        </CardHeader>
      ...
      {/* Billing Rates / Model Pricing Card */}
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          >
            <RefreshCw className={`h-4 w-4 mr-1 ${pricingLoading ? 'animate-spin' : ''}`} />
            Refresh
          </Button>
        </CardHeader>
        <CardContent>
          {pricingError && (
            <Alert variant="destructive" className="mb-4">
              <AlertDescription>
                {pricingError instanceof Error ? pricingError.message : 'Failed to load pricing'}
              </AlertDescription>
            </Alert>
          )}
          ...
          {pricingLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-4">
              No model pricing configured. Pricing will be seeded when the database initializes.
            </p>
          ) : (
            <div className="space-y-6">
              {Object.entries(pricingByProvider).map(([provider, models]) => (
                        ...
                        {models.map((pricing) => (
                          <TableRow key={pricing.id}>
                            ...
                            <TableCell className="text-right font-medium">
                              {formatCost(pricing.input_cost_per_million)}
                            </TableCell>
                            <TableCell className="text-right font-medium">
                              {formatCost(pricing.output_cost_per_million)}
                            </TableCell>
                            <TableCell className="text-right text-primary">
                              {calculateVibePerMillion(pricing.input_cost_per_million).toLocaleString()}
                            </TableCell>
                            <TableCell className="text-right text-primary">
                              {calculateVibePerMillion(pricing.output_cost_per_million).toLocaleString()}
                            </TableCell>
