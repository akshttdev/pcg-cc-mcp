...
import { type BusinessReportRecord, reportsApi } from '@/lib/api';
...
export function ReportDetail() {
  ...
  const patchMut = useMutation({
    mutationFn: (data: Partial<BusinessReportRecord>) =>
      reportsApi.patch(id!, data),
    onSuccess: (updated) => {
  ...
  if (!report)
    return <div className="p-8 text-slate-500">Report not found.</div>;
  ...
  const recommendedServices = parseJson<
    Array<{ name: string; rationale: string; timeline: string }>
  >(report.recommended_services, []);
  const nextSteps = parseJson<
    Array<{ action: string; owner: string; deadline: string }>
  >(report.next_steps, []);
  const individuals = parseJson<
    Array<{
      name: string;
      role: string;
      company: string;
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8 print:px-0 print:py-0">
      ...
      <div className="flex items-start justify-between gap-4 print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href={`/api/business-reports/${id}/pdf`}
            target="_blank"
          ...
          >
            <Button
              variant="outline"
              size="sm"
              className="gap-2 border-emerald-700/60 text-emerald-400 hover:bg-emerald-950/40"
            >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400 mb-2">
                    ...
                    {ind.role && ind.company && (
                      <span className="mx-1.5">·</span>
                    )}
                    {ind.company && <span>{ind.company}</span>}
                  </p>
                  {ind.summary && (
                    <p className="text-sm text-slate-300 mb-1">{ind.summary}</p>
                  )}
                  {ind.key_insights && (
