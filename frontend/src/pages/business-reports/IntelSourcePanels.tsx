...
export function IntelSourcePanels({
  personId,
  companyId,
}: IntelSourcePanelsProps) {
      ...
      {person && (
        <div
          className={`rounded-xl border p-4 space-y-3 transition-all ${
            flagged.has('person')
              ? 'border-red-800 bg-red-950/20 opacity-60'
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {person.intelligence_status === 'done' && (
                <Badge
                  variant="outline"
                  className="text-xs px-1.5 border-emerald-700 text-emerald-400"
                >
                  verified
                </Badge>
              )}
            ...
            <div className="flex items-center gap-2">
              <Link
                to={`/contacts/${person.id}?tab=reports`}
              ...
              <button
                ...
                title={
                  flagged.has('person') ? 'Remove flag' : 'Flag as unreliable'
                }
          ...
          <div>
            ...
            {person.job_title && (
              <p className="text-xs text-slate-400">{person.job_title}</p>
            )}
          </div>
          ...
          {hasPersonIntel ? (
            ...
            <p className="text-xs text-slate-600 italic">
              No intelligence data yet — run research from the person profile.
            </p>
          )}

          {person.intelligence_confidence != null &&
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.round((person.intelligence_confidence ?? 0) * 100)}%`,
                    }}
                  />
                </div>
                <span className="text-xs text-slate-500">
                  {Math.round((person.intelligence_confidence ?? 0) * 100)}%
                  confidence
                </span>
              </div>
            )}
      ...
      {company && (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={
                  flagged.has('company') ? 'Remove flag' : 'Flag as unreliable'
                }
          ...
          {hasCompanyIntel ? (
            ...
            <p className="text-xs text-slate-600 italic">
              No brand intelligence yet — run research from the company profile.
            </p>
          )}

          {company.intelligence_confidence != null &&
            company.intelligence_confidence > 0 && (
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-violet-500 rounded-full"
                    style={{
                      width: `${Math.round((company.intelligence_confidence ?? 0) * 100)}%`,
                    }}
                  />
                </div>
                <span className="text-xs text-slate-500">
                  {Math.round((company.intelligence_confidence ?? 0) * 100)}%
                  confidence
                </span>
              </div>
            )}
