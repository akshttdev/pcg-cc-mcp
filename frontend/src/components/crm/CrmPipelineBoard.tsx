...
import {
  useCreateDeal,
  useCrmKanban,
  useCrmPipelineByType,
  useDeleteDeal,
  useMoveDeal,
  useOrgCrmKanban,
  useOrgCrmPipelineByType,
  useUpdateDeal,
} from '@/hooks/useCrmPipeline';
...
function getStageOwner(
  ...
  if (name === 'discovery')
    return { label: 'Account Manager + Nora', type: 'team' };
  ...
  if (name === 'proposal meeting')
    return { label: 'Account Manager', type: 'human' };
...
function StageOwnerBadge({ owner }: { owner: StageOwner }) {
  const Icon =
    owner.type === 'agent' ? Bot : owner.type === 'team' ? Users : User;
  const typeLabel =
    owner.type === 'agent'
      ? 'AI agent'
      : owner.type === 'team'
...
const STAGE_DESCRIPTIONS: Record<string, string> = {
  // 9-Stage Dealflow
  intel:
    'Scout runs Phase I — Who-Is on person + company. Person intel wiki & company intel wiki auto-generated.',
  'business analysis':
    'Astra runs Phase II — comprehensive business research. Business Report Artifact generated, discovery call scheduled.',
  discovery:
    'Discovery call with the prospect. Nora auto-links transcript. Out-of-order flow triggers Scout if no intel exists.',
  proposal:
    'Cash generates AI proposal from full knowledge graph — business report, discovery transcript, intel wikis. Operator reviews + approves.',
  polish:
    'Lux generates branded sales deck from approved proposal + org & client brand guides. Operator reviews before presenting.',
  present:
    'Present deck to client on live call, then send invoice and confirm payment.',
  'present & invoice':
    'Present deck to client on live call, then send invoice and confirm payment.',
  'follow up':
    'Post-presentation follow-up. Nora sends follow-up comms. Close out as Won or Lost.',
...
export function CrmPipelineBoard({
  ...
  useEffect(() => {
    ...
    for (const stageData of kanbanData.stages) {
      const updated: CrmDealWithContact | undefined = stageData.deals.find(
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const targetStage = kanbanData.stages.find(
      (s) => s.stage.id === targetStageId
    );
    const newPosition = targetStage?.deals.length ?? 0;

    moveDeal.mutate({
      dealId,
      data: { stage_id: targetStageId, position: newPosition },
    });
  };
  ...
  const stages = useMemo(
    () => kanbanData?.stages.map((s) => s.stage) ?? [],
    [kanbanData]
  );
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-3 py-12">
        ...
        <p className="text-sm text-center max-w-md">
          This organization doesn&apos;t have a deal pipeline set up yet. Create
          one to start tracking deals through your sales process.
        </p>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-xl font-bold">
              {title || kanbanData.pipeline_name}
            </h1>
      ...
      <CrmDealDetailPanel
        ...
        onMoveTo={(deal, stageId) => {
          ...
          moveDeal.mutate({
            dealId: deal.id,
            data: { stage_id: stageId, position: targetSD?.deals.length ?? 0 },
          });
        }}
      />
    </div>
