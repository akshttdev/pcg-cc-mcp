...
test.describe('Deal API — Data Integrity', () => {
  test('rich deal endpoint returns person_id and company intel', async ({ page }) => {
...
test.describe('Pipeline Stages — Configuration', () => {
  test('pipeline has all 9 stages with correct stage_types', async ({ page }) => {
    ...
    console.log('Stages returned:', stages.map((s: { name: string; stage_type: string }) => `${s.name} (${s.stage_type})`));

    for (const expected of expectedStages) {
      const found = stages.find((s: { name: string; stage_type: string }) =>
        s.name.toLowerCase().includes(expected.name.toLowerCase()) ||
        s.stage_type === expected.type
      );
      expect(found, `Stage "${expected.name}" with type "${expected.type}" not found`).toBeTruthy();
    }
...
test.describe('CRM Pipeline Board UI', () => {
  test('board loads and deal card is present in DOM', async ({ page }) => {
    ...
    await page.waitForSelector('[class*="kanban"], [class*="column"], [class*="stage"], [class*="pipeline"]', { timeout: 25000 })
      .catch(() => null);
    await page.waitForTimeout(3000);

    // Deal may be in Proposal column — check DOM presence (any mention)
  ...
  test('deal card contains proposal chip in DOM', async ({ page }) => {
    ...
    const hasProposal = bodyText?.includes('Proposal') || bodyText?.includes('proposal');
    console.log('✅ Page has proposal content:', hasProposal);
    expect(bodyText?.length).toBeGreaterThan(100); // page loaded
  });

  test('clicking deal card via API navigation opens detail panel', async ({ page }) => {
...
test.describe('Deal Detail Panel — Tabs', () => {
  ...
  test('Intel tab shows person and company intel', async ({ page }) => {
    ...
    await page.locator('[role="tablist"] button, [role="tab"]').filter({ hasText: /^Intel$/ }).first().click();
    await page.waitForTimeout(1000);

    // Should show contact name
    await expect(page.locator('text=Joshua Marotta').first()).toBeVisible({ timeout: 5000 });
  ...
  test('Intel tab company profile link goes to /companies/, not /persons/', async ({ page }) => {
    await openDealPanel(page);
    await page.locator('[role="tablist"] button, [role="tab"]').filter({ hasText: /^Intel$/ }).first().click();
    await page.waitForTimeout(1000);
  ...
  test('Transcripts tab shows linked discovery call transcript', async ({ page }) => {
    await openDealPanel(page);
    await page.locator('[role="tablist"] button, [role="tab"]').filter({ hasText: /^Transcripts$/ }).first().click();
    await page.waitForTimeout(1000);
  ...
  test('Proposal tab shows proposal text', async ({ page }) => {
    await openDealPanel(page);
    await page.locator('[role="tablist"] button, [role="tab"]').filter({ hasText: /^Proposal$/ }).first().click();
    await page.waitForTimeout(1000);

    // Should show proposal content (proposal text rendered in a monospace block)
    await expect(page.locator('text=BRAND IDENTITY').first()).toBeVisible({ timeout: 5000 });
    await expect(page.locator('text=Executive Summary').first()).toBeVisible({ timeout: 5000 });
  ...
  test('Deck & Close tab shows deck generation and won sections', async ({ page }) => {
    await openDealPanel(page);
    await page.locator('[role="tablist"] button, [role="tab"]').filter({ hasText: /Deck/i }).first().click();
    await page.waitForTimeout(1000);

    // Should show the three sections
    await expect(page.locator('text=Sales Deck').first()).toBeVisible({ timeout: 5000 });
    await expect(page.locator('text=Invoice').first()).toBeVisible({ timeout: 5000 });
    await expect(page.locator('text=Close Deal').first()).toBeVisible({ timeout: 5000 });
...
test.describe('UI Enhancement Checks', () => {
  ...
  test('pipeline stepper shows current stage', async ({ page }) => {
    ...
    expect(['Intel', 'Business Analysis', 'Discovery', 'Proposal', 'Polish', 'Present', 'Follow Up', 'Won', 'Lost'])
  ...
  test('person profile link navigates to /persons/:id', async ({ page }) => {
    ...
    expect(personRes.data?.full_name || personRes.full_name).toContain('Marotta');
  ...
  test('no broken navigation links (company vs person)', async ({ page }) => {
    ...
    await page.locator('text=Hudson\'s Car Club').first().scrollIntoViewIfNeeded();
    await page.locator('text=Hudson\'s Car Club').first().click({ force: true });
    await page.waitForFunction(() => document.querySelectorAll('[data-state="open"]').length > 0, { timeout: 10000 });
    ...
    const linkHrefs = await Promise.all(allLinks.map(l => l.getAttribute('href')));

    const companyLinks = linkHrefs.filter(h => h?.includes('/companies/'));
    const personLinks = linkHrefs.filter(h => h?.includes('/persons/'));
    const reportLinks = linkHrefs.filter(h => h?.includes('/business-reports/'));
